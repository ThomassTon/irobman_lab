...
PlanResult plan_single_arm_given_subsequence_and_prev_plan(    rai::Configuration C, const RobotTaskPoseMap &rtpm,
...
PlanResult plan_single_arm_given_sequence(
    rai::Configuration C, const RobotTaskPoseMap &rtpm,
    const TaskSequence &sequence, const std::map<Robot, arr> &home_poses,
    const uint best_makespan_so_far = 1e6) {

  Plan paths;
  return plan_single_arm_given_subsequence_and_prev_plan(
      C, rtpm, sequence, 0, paths, home_poses, best_makespan_so_far);
}
